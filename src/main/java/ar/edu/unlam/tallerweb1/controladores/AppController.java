...
public class AppController {
	...
	public ModelAndView productDetail(@RequestParam Long idItem, @RequestParam String category,
			@RequestParam Double latitude, @RequestParam Double longitude, @RequestParam Long distance) {
		ModelMap model = new ModelMap();
	...
	public ModelAndView reserve(@RequestParam Long idCommerce, @RequestParam Long idItem, @RequestParam Double latitude,
	...
	public ModelAndView saveReserve(@ModelAttribute("reserve") Reserve reserve) {
		...
		if (stock - reserve.getAmount() < 0) {
			...
			String message = String.format("Se ha reservado el producto %s por una cantidad de %d en el comercio %s",
					reserve.getItem().getDescription(), reserve.getAmount(), reserve.getCommerce().getName());
